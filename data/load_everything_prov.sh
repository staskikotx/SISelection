...
for db_dir in "$DEV_DATABASES_DIR"/*; do
    ...
    psql -d "$pg_db_name" -c "CREATE EXTENSION IF NOT EXISTS \"uuid-ossp\" " 2>&1
    psql -d "$pg_db_name" -c "DROP EXTENSION IF EXISTS provsql" 2>&1
    psql -d "$pg_db_name" -c "CREATE EXTENSION IF NOT EXISTS provsql" 2>&1
    ...
    psql -d "$pg_db_name" -c "\i /home/ashulgin/material/alpha-sql/bird/add_provenance.sql" 2>&1
done

echo -e "\nProcess complete! Summary:"
echo "• Total databases found: $(find "$DEV_DATABASES_DIR" -mindepth 1 -maxdepth 1 -type d | wc -l)"
echo "• Successfully loaded: $(psql -lqtA | grep '^prov_' | wc -l)"
