    ...
    def run(self) -> None:
        ...
        try:
            ...
            with sqlite3.connect(f'file:{self.db_path}?mode=ro', uri=True) as conn:
                ...
                #logger.info(f"executing SQL {self.query} have result {self.result}")
        except Exception as e:
            self.exception = e
            logger.info(f"execting SQL {self.query} have error *********** {e} ***********")

class ExecuteSQLOnStringThread(threading.Thread):
    """
    Thread to execute a SQL query on an in-memory database initialized from a string.
    """
    def __init__(self, setup_sql: str, query: str, timeout: int) -> None:
        super().__init__()
    ...
    def run(self) -> None:
        ...
        try:
            with sqlite3.connect(':memory:') as conn:
                ...
                # Step 1: Initialize the in-memory database
                cursor.executescript(self.setup_sql)

                # Step 2: Execute the target query
                cursor.execute(self.query)

                # Step 3: Capture result metadata and data
                self.result_cols = [col[0] for col in cursor.description]
                self.result = cursor.fetchall()
                logger.info(f"Executing SQL '{self.query}' returned {len(self.result)} rows")

        except Exception as e:
            self.exception = e
            logger.error(f"Error executing SQL '{self.query}': {e}")

def execute_sql_on_string_with_timeout(setup_sql: str, query: str, timeout: int = 60) -> SQLExecutionResult:
    """
    Execute a SQL query synchronously with a timeout on an in-memory database.

    Args:
        setup_sql: A string containing SQL statements to initialize the in-memory database (e.g., CREATE TABLE and INSERT).
        query: The SQL query to execute.
        timeout: The timeout in seconds.

    Returns:
        The result of the SQL query as an SQLExecutionResult object.
    """
    thread = ExecuteSQLOnStringThread(setup_sql, query, timeout)
    thread.daemon = True
    thread.start()
    thread.join(timeout)
    ...
    if thread.is_alive():
        ...
        return SQLExecutionResult(":memory:", query, SQLExecutionResultType.TIMEOUT, None, None, error_message)

    if thread.exception:
        error_message = str(thread.exception)
        return SQLExecutionResult(":memory:", query, SQLExecutionResultType.ERROR, None, None, error_message)

    return SQLExecutionResult(":memory:", query, SQLExecutionResultType.SUCCESS, thread.result_cols, thread.result, None)
...
def measure_sql_execution_time(db_path: str, sql_query: str, repeat: int = 3) -> float:
    execution_times = []
